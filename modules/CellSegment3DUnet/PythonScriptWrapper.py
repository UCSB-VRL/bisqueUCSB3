...
class PythonScriptWrapper(object):
	...
	This will select seed slice index and provide a black threshold for mask creation
        ...
        output_files, adj_table, points, cell_vol, coordinates, center, segments = postprocessing.main(bq,
                                                                                                       prob_map_datadir,
                                                                                                       results_outdir,
                                                                                                       testing_data_dir,
                                                                                                       min_distance,
    ...
    def run(self):
        ...
        try:
            ...
            self.outfiles, self.outtable_xml_adj_table, self.outtable_xml_points, self.outtable_xml_cell_vol, self.outtable_xml_coordinates, self.outtable_xml_center, self.segments_xml = self.postprocess(
                bq)
        except (Exception, ScriptError) as e:
        ...
        str(self.segments_xml), str(self.outtable_xml_adj_table), str(self.outtable_xml_points),
        str(self.outtable_xml_cell_vol), str(self.outtable_xml_coordinates), str(self.outtable_xml_center),
        self.restable.get('value'))
