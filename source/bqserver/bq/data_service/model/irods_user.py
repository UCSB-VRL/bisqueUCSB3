...
class BisQueIrodsIntegration:
    def __init__(
        self, host="localhost", port=1247, admin_user="", password="", zone=""
    ):
        self.host = host
        self.port = port
    ...
    def load_from_env(self):
        ...
        if len(admin_user) == 0:
            raise ValueError(
                "Environment varaible 'BISQUE_IRODS_ADMIN_USERNAME' is not set"
            )

        if len(password) == 0:
            raise ValueError(
                "Environment varaible 'BISQUE_IRODS_ADMIN_PASSWORD' is not set"
            )
...
def main(argv):
    ...
    integ = BisQueIrodsIntegration(
        host=arg["host"],
        port=arg["port"],
        admin_user=arg["admin_user"],
