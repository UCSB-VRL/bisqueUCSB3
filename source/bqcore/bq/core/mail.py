...
class EmailConfiguration:
    ...
    def _load_from_tg_config(self) -> bool:
        ...
        log.debug(f"TG config loading: found_config={found_config}, smtp_host={getattr(self, 'smtp_host', None)}")
        ...
        if found_config and self.smtp_host:
            ...
            if self.smtp_username and self.smtp_password:
                ...
                log.debug(f"SMTP configuration incomplete - host: {bool(self.smtp_host)}, username: {bool(self.smtp_username)}, password: {bool(self.smtp_password)}")
    ...
    def _load_from_config_file(self) -> bool:
        ...
        try:
            import configparser
            config_file = '/media/wskoly/7A0AD0890AD04433/LinuxCodes/bisqueUCSB3/source/config/site.cfg'
            ...
            for config_key, attr_name in config_map.items():
                if parser.has_option(section, config_key):
                    ...
                    if value is not None:
                        value = value.strip()
                        if value or attr_name in ['smtp_username', 'smtp_password']:  # Allow empty username/password
                            found_config = True
                            if attr_name in ['smtp_port', 'smtp_timeout']:
            ...
            if found_config and self.smtp_host:
                self.smtp_port = getattr(self, 'smtp_port', None) or (587 if getattr(self, 'smtp_use_tls', False) else 25)
                self.default_from_email = getattr(self, 'default_from_email', None) or getattr(self, 'admin_email', None)
...
class EmailService:
    ...
    def __init__(self, config: Optional[EmailConfiguration] = None):
        log.info(f"Initializing EmailService with config: {config} {config.get_config_summary() if config else 'None'}")
