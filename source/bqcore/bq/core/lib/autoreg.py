...
class AutoRegister(object):
    ...
    def login_group(self, login_identifier):
        ...
        g = (
            model.DBSession.query(model.Group)
            .filter_by(group_name=login_identifier)
            .first()
        )
        if g is None:
            g = model.Group()
    ...
    def validate_user(self, user_name, values):
        ...
        if name_match is not None:
            log.info(
                "found existing user: name %s by user %s ", user_name, str(name_match)
            )
            return user_name
        return None
    ...
    def register_user(self, user_name, values):
        ...
        email_match = values.get("email_address") and model.User.by_email_address(
            values["email_address"]
        )
        if email_match is not None:
            log.info(
                "found existing user: name %s (%s) by email %s ",
                user_name,
                email_match.user_name,
                str(email_match),
            )
    ...
    def add_metadata(self, environ, identity):
        ...
        try:
            from bqserver.bq.data_service.model import BQUser

            if user:
                bquser = (
                    model.DBSession.query(BQUser).filter_by(resource_name=user).first()
                )
