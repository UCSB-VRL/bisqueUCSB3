...
class OpenIDAuth(object):
    ...
    def _auto_register(self, environ, identity, user_name):
        ...
        if registration:
            ...
            if "repoze.who.plugins.openid.lastname" in identity:
                name = "%s %s" % (
                    name,
                    identity["repoze.who.plugins.openid.lastname"][0],
                )
            ...
            self.log.debug(
                "%s not found in %s"
                % (self.auto_register, environ["repoze.who.plugins"])
            )
        return user_name
    ...
    def authenticate(self, environ, identity):
        ...
        if "repoze.who.plugins.openid.email" in identity:
            self.log.info(
                "authenticated email: %s " % identity["repoze.who.plugins.openid.email"]
            )
        ...
        if "repoze.who.plugins.openid.userid" in identity:
            self.log.info(
                "authenticated : %s " % identity["repoze.who.plugins.openid.userid"]
            )
