...
class CASPlugin(object):
    ...
    def challenge(self, environ, status, app_headers, forget_headers):
        ...
        # else:
        # if login_type == 'cas':
        if request.path == self.login_path and environ.get(
            "repoze.who.plugins.cas"
        ):  #  and request.params.get('login_type', None)=='cas':
            log.debug("CAS challenge redirect to %s", self.cas_login_url)
            destination = "%s?service=%s" % (
                self.cas_login_url,
...
""" % (
    ...
    def _validate_saml(self, environ, identity):
        ...
        if "repoze.who.plugins.cas.ticket" in identity:
            ...
            if (
                response.status_code == requests.codes.ok and found
    ...
    def _validate_register(self, environ, identity, user_id):
        ...
        if plugin:
            ...
            if validate_method is None or not callable(validate_method):
                log.error(
                    "%s is not a callable method on plugin %s",
        ...
        log.debug(
            "%s not found in %s. Ensure autoreg is enabled in who.ini",
            self.validate_plugin,
