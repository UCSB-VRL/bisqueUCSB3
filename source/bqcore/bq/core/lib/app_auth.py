...
class AppAuthPlugin(object):
    ...
    def challenge(self, environ, status, app_headers, forget_headers):
        ...
        if any(
            content_type.startswith(v) for v in ("application/json", "application/xml")
        ):
    ...
    def identify(self, environ):
        ...
        if request.path == self.login_path:
            ...
            if content_type.startswith("application/json"):
                ...
                login_data = dict(
                    username=xml.find("tag[@name='username']").get("value"),
            ...
            return {
                "login": login_data.get("username", None),
