...
class MexAuthenticatePlugin(object):
    ...
    def authenticate(self, environ, identity):
        ...
        if not asbool(config.get("bisque.has_database")):
            ...
            identity["bisque.mex_auth"] = "%s:%s" % (
                identity.get("bisque.mex_user"),
                mex_token,
            )
            log.debug("NO DB mex_auth=%s", identity["bisque.mex_auth"])
            return mex_token

        from bqserver.bq.data_service.model import ModuleExecution
        ...
        try:
            ...
            mex = (
                ...
                .filter_by(resource_uniq=mex_token)
                .first()
            )
        ...
        if mex:
            ...
            if request_owner is not None and owner != request_owner:
                log.error(
                    "Mex with bad owner reuqest %s != mex %s", request_owner, owner
                )
                return None
