...
def bootstrap(command, conf, vars):
    ...
    from bqserver.bq.data_service.model import Taggable, Tag, BQUser, ModuleExecution
    ...
    try:
        ...
        admin._email_address = config.get(
            "bisque.admin_email", "manager@somedomain.com"
        )
    ...
    try:
        ...
        system = (
            model.DBSession.query(Taggable).filter_by(resource_type="system").first()
        )
        if system is None:
            ...
            if os.path.exists(system_prefs):
                ...
                print(
                    (
                        "Couldn't find %s: using minimal default preferences"
                        % system_prefs
                    )
                )
        ...
        log.exception(
            "Warning, there was a problem adding your system object, it may have already been added:"
        )
        # import traceback
        # print traceback.format_exc()
        transaction.abort()
