...
try:
    from bqcore.bq.core.identity import add_credentials
except ImportError:
...
def prepare_credentials(client, headers, userpass=None):
    if userpass is not None:
        ...
        headers["authorization"] = (
            "Basic " + base64.encodestring("%s:%s" % userpass).strip()
        )
        return
    cred = {}
...
def request(
    ...
    **kw
    ...
    if _async and async_request_pool.isrunning():
        response_tuple = async_request_pool.request(
            url,
            callback=callback,
            method=method,
            body=body,
        ...
        response_tuple = sync_request.request(
            url,
            method=method,
            body=body,
            headers=headers,
...
def send(req):
    ...
    headers, content = request(
        req.url, method=req.method, headers=req.headers, body=req.body
    )
