...
from bqcore.bq.util.diskusage import disk_usage

__all__ = ["clean_images"]


def clean_store(stores, options):
    from bqserver.bq.data_service.model import Taggable, Image, DBSession
...
def clean_images(options):
    ...
    # stores = load_stores()
    from bqserver.bq.blob_service.controllers.mount_service import load_default_drivers
    ...
    clean_store(
        [
            store
            for name, store in list(drivers.items())
            if store["mounturl"].startswith("file")
        ],
        options,
    )
