...
def upload_file(url, filename, user="", passwd="", opts=None):
    ...
    try:
        if not opts.dryrun:
            ...
            resp = (
                urllib.request.build_opener(
                    urllib.request.HTTPRedirectHandler, MultipartPostHandler, auth
                )
                .open(url, data)
                .close()
            )
        ...
        raise Abort(
            "Problem uploading %s to %s (try it manually using a web browser): %s\n"
            % (filename, url, err)
        )
...
import mimetypes, email.generator
import os, stat
...
def main():
    ...
    for filename in args:
        if os.path.exists(filename):
            ...
            fname = upload_file(
                opts.destination, filename, opts.user, opts.password, opts
            )
