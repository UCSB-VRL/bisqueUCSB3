...
try:
    ...
    ms = magic.Magic(flags=magic.MAGIC_NONE)

    def isimagefile(fname: str) -> bool:
        """
        Check if a file is an image based on its content.

        :param fname: Path to the file.
        :return: True if the file is an image, False otherwise.
        """
        ftype = ms.from_file(fname)
        return "image" in ftype.lower()
...
def upload(dest, filename, userpass, tags=None):
    ...
    if response.status_code != 200:
        print(
            (
                "error while copying %s: Server response %s"
...
def main():
    ...
    config.read(
        [".bisque", os.path.expanduser("~/.bisque"), "/etc/bisque/bisque_config"]
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="recurse into dirs",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="print actions"
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", default=False, help="print debug log"
    )
    parser.add_argument(
        "-t", "--tag", action="append", dest="tags", help="-t name:value"
    )
    parser.add_argument(
        "--resource",
        action="store",
        default=None,
        help="XML resource record for the file",
    )
    parser.add_argument(
        "--dest", default=defaults["bisque_host"], help="Bisque server root"
    )
    parser.add_argument(
        "--ingest-type",
        default=None,
        help="Upload special type of image .. %s " % ",".join(SPECIAL_TYPES),
    )
