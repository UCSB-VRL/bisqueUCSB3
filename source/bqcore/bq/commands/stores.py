...
def _create_default_mounts(drivers, root=None):
    ...
    if root is None:
        ...
        etree.SubElement(
            root, "tag", name="order", value=",".join(list(drivers.keys()))
        )
...
def _update_mounts(drivers):
    ...
    if storeorder is None:
        log.warn("order tag missing from root store adding")
        storeorder = etree.SubElement(
            user_root, "tag", name="order", value=",".join(list(drivers.keys()))
        )
        update = True
    elif len(storeorder) == 1:
        storeorder = storeorder[0]
    ...
    if update:
        return data_service.update(
            user_root, new_resource=user_root, replace=False, view="full"
        )
