...
def logger_command(command, cfgopt, processes):
    ...
    if command is "start":
        with open(os.devnull, "w") as fnull:
            ...
            logger = Popen(
                [exe_path(launcher[0])] + launcher[1:], shell=(os.name == "nt")
            )
...
BACKENDS = {"paster": paster_command, "uwsgi": uwsgi_command}
...
def operation(command, options, *args):
    ...
    try:
        ...
        for key, serverspec in sorted(config["servers"].items()):
            ...
            cfgopt["logfile"] = os.path.join(
                config["log_dir"], LOG_TEMPL % cfgopt["port"]
            )
            cfgopt["pidfile"] = os.path.join(
                config["pid_dir"], PID_TEMPL % cfgopt["port"]
            )
            ...
            if command in ("stop", "restart"):
                ...
                if error is not None and backend == "paster":
                    ...
                    for proc in processes:
                        proc.wait()
                    print(
                        "Recovered Paste windows error and successfully stopped daemon"
                    )
            ...
            if command in ("start", "restart"):
                if os.path.exists(cfgopt["pidfile"]):
                    try:
                        ...
                        if psutil.pid_exists(pid):
                            if options.force:
                                print(
                                    "old pid file: %s exists! restarting..."
                                    % cfgopt["pidfile"]
                                )
                ...
                processes = backend_command(
                    "start", options, cfgopt, processes, cfg_file
                )
