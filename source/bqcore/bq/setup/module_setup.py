...
from bqcore.bq.util.copylink import copy_link
from bqcore.bq.util.paths import find_config_path, defaults_path
...
def needs_update(output, dependency):
    ...
    return not os.path.exists(output) or (
        os.path.exists(dependency)
        and os.path.getmtime(dependency) > os.path.getmtime(output)
    )
...
def ensure_matlab(params):
    ...
    if matlab_home not in os.environ["PATH"]:
        os.environ["PATH"] = (
            os.path.join(matlab_home, "bin") + os.pathsep + os.environ["PATH"]
        )
...
def mcc(command, *largs, **kw):
    ...
    try:
        ...
        print(
            "Please check your matlab enviroment. In particular check if mcc is available and callable from the shell"
        )
        return False
    if where:
        os.chdir(cwd)
...
def matlab_setup(
    main_path, files=[], bisque_deps=False, dependency_dir="mbuild", params={}, **kw
):
    ...
    ctf = main_name + ".ctf"
    if not (
        needs_update(ctf, source)
        or needs_update(main, source)
        or needs_update(ctf, "+bq")
        or needs_update(ctf, "+bim")
    ):
...
def python_setup(scripts, package_scripts=True, dependency_dir="pydist", params=None):
    ...
    filename = find_config_path(
        "templates/python_launcher.tmpl", config_dir="config-defaults"
    ).replace("\\", "/")
...
def docker_setup(image, command, base_image=None, params=None):
    ...
    from bqengine.bq.engine.controllers.command_run import BaseRunner
