...
import urllib.parse
import zipfile
from bqapi.bqapi import BQSession, BQCommError
from bqapi.bqapi.util import save_blob  # local
import posixpath
import sys
...
def image_tiles(bqsession, image_service_url, tile_size=64):
    ...
    meta = bqsession.fetchxml(
        urllib.parse.urlunparse(
            [o.scheme, o.netloc, o.path, o.params, meta_query, o.fragment]
        )
    )
    ...
    for ix in range(int(x / tile_size) - 1):
        for iy in range(int(y / tile_size) - 1):
            ...
            yield urllib.parse.urlunparse(
                [o.scheme, o.netloc, o.path, o.params, tile_query, o.fragment]
            )
...
def upload_image_resource(bqsession, path, filename):
    ...
    print(
        "Uploaded id: %s url: %s" % (content.get("resource_uniq"), content.get("uri"))
    )
    return content
...
def setup_simple_feature_test(ns):
    ...
    feature_response_results = (
        config.get("SimpleTest", "feature_response") or DEFAULT_FEATURE_RESPONSE_HDF5
    )
    feature_past_response_results = (
        config.get("SimpleTest", "feature_sample") or DEFAULT_FEATURE_SAMPLE_HDF5
    )
...
def setup_parallel_feature_test(ns):
    ...
    feature_response_results = (
        config.get("ParallelTest", "feature_response")
        or DEFAULT_FEATURE_PARALLEL_RESPONSE_HDF5
    )
    feature_past_response_results = (
        config.get("ParallelTest", "feature_sample")
        or DEFAULT_FEATURE_PARALLEL_SAMPLE_HDF5
    )
    ...
    test_image_location = fetch_file(
        test_image, ns.store_location, ns.store_local_location
    )
...
def setup_image_upload(ns):
    ...
    """
    content = upload_image_resource(
        ns.session, ns.test_image_location, "%s/%s" % (TEST_PATH, ns.test_image)
    )
...
def setup_dataset_upload(ns):
    ...
    image_uri1 = "%s/image_service/image/%s" % (
        ns.root,
        content.attrib["resource_uniq"],
    )
    ...
    image_uri2 = "%s/image_service/image/%s" % (
        ns.root,
        content.attrib["resource_uniq"],
    )
    ...
    image_uri3 = "%s/image_service/image/%s" % (
        ns.root,
        content.attrib["resource_uniq"],
    )
...
def setup_mask_upload(ns):
    ...
    mask[size_y / 4 : (3 * size_y) / 4, size_x / 4 : (3 * size_x) / 4] = (
        1  # create mask
    )
    ...
    content = upload_image_resource(
        ns.session,
        os.path.join(ns.store_local_location, maskname),
        "%s/%s" % (TEST_PATH, maskname),
    )
...
def setup_polygon_upload(ns):
    ...
    vertices[:, 0] = vertices[:, 0] + size_y * 0.5  # center y
    vertices[:, 1] = vertices[:, 1] + size_x * 0.5  # center x
...
def setup_circle_upload(ns):
    ...
    vertices = np.array([[0, 0], [-0.3, 0.3]])  # shape
