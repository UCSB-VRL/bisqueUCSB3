...
def test_simple_test_feature_resource_type_not_found_xml():
    ...
    response_code = 200
    request = (
        "%s/features/SimpleTestFeature/xml?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_simple_test_feature_resource_type_not_found_csv():
    ...
    response_code = 200
    request = (
        "%s/features/SimpleTestFeature/csv?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_simple_test_feature_resource_type_not_found_hdf():
    ...
    response_code = 200
    request = (
        "%s/features/SimpleTestFeature/hdf?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_test_feature_resource_type_not_found_xml():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedTestFeature/xml?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_test_feature_resource_type_not_found_csv():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedTestFeature/csv?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_test_feature_resource_type_not_found_hdf():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedTestFeature/hdf?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_parameter_test_feature_resource_type_not_found_xml():
    ...
    response_code = 200
    request = (
        "%s/features/ParametersTestFeature/xml?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_parameter_test_feature_resource_type_not_found_csv():
    ...
    response_code = 200
    request = (
        "%s/features/ParametersTestFeature/csv?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_parameter_test_feature_resource_type_not_found_hdf():
    ...
    response_code = 200
    request = (
        "%s/features/ParametersTestFeature/hdf?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_parameter_test_feature_resource_type_not_found_xml():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedParametersTestFeature/xml?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_parameter_test_feature_resource_type_not_found_csv():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedParametersTestFeature/csv?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_uncached_parameter_test_feature_resource_type_not_found_hdf():
    ...
    response_code = 200
    request = (
        "%s/features/UncachedParametersTestFeature/hdf?image=%s/image_service/image/notaresource"
        % (NS.root, NS.root)
    )
    check_response(NS.session, request, response_code)
...
def test_simple_test_feature_gobject_not_found():
    ...
    response_code = 200
    request = (
        "%s/features/SimpleTestFeature/xml?image=%s&gobject=%s/image_service/image/notaresource"
