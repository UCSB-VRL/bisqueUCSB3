...
def test_get_simple_test_feature_csv_with_mask_and_gobject():
    ...
    response_code = 200
    request = "%s/features/SimpleTestFeature/csv?image=%s&mask=%s&gobject=%s" % (
        NS.root,
        NS.image_uri,
        NS.image_uri,
        NS.image_uri,
...
def test_get_uncached_test_feature_xml_with_mask_and_gobject():
    ...
    response_code = 200
    request = "%s/features/UncachedTestFeature/xml?image=%s&mask=%s&gobject=%s" % (
        NS.root,
        NS.image_uri,
        NS.image_uri,
        NS.image_uri,
...
def test_get_uncached_test_feature_csv_with_mask_and_gobject():
    ...
    response_code = 200
    request = "%s/features/UncachedTestFeature/csv?image=%s&mask=%s&gobject=%s" % (
        NS.root,
        NS.image_uri,
        NS.image_uri,
        NS.image_uri,
...
def test_get_multivector_test_feature_xml():
    ...
    response_code = 200
    request = "%s/features/MultiVectorTestFeature/xml?image=%s" % (
        NS.root,
        NS.image_uri,
    )
    check_response(NS.session, request, response_code)
...
def test_get_multivector_test_feature_hdf():
    ...
    response_code = 200
    request = "%s/features/MultiVectorTestFeature/hdf?image=%s" % (
        NS.root,
        NS.image_uri,
    )
    check_response(NS.session, request, response_code)
...
def test_get_uncached_multivector_test_feature_xml():
    ...
    name = "get_multivector_test_feature_xml"
    response_code = 200
    request = "%s/features/UncachedMultiVectorTestFeature/xml?image=%s" % (
        NS.root,
        NS.image_uri,
    )
    check_response(NS.session, request, response_code)
...
def test_get_uncached_multivector_test_feature_csv():
    ...
    name = "get_multivector_test_feature_csv"
    response_code = 200
    request = "%s/features/UncachedMultiVectorTestFeature/csv?image=%s" % (
        NS.root,
        NS.image_uri,
    )
    check_response(NS.session, request, response_code)
