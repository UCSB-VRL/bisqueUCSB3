...
import os
import uuid
from bqapi.bqapi import BQCommError
from bqapi.bqapi.bqfeature import ParallelFeature, FeatureResource
...
def check_feature(ns, test, feature_name, image=None, mask=None, gobject=None):
    ...
    try:
        resources = []
        if image:
            resources.append("image=%s" % image)
        if mask:
            resources.append("mask=%s" % mask)
        if gobject:
            resources.append("gobject=%s" % gobject)
        query = "&".join(resources)
        request = "%s/features/%s/hdf?%s" % (ns.root, feature_name, query)
        temp_response_path = ns.session.c.fetch(
            request,
            headers={"Content-Type": "text/xml", "Accept": "text/xml"},
    ...
    past_results_table_path = os.path.join(
        ns.store_local_location, ns.feature_past_response_results
    )
    ...
    with tables.open_file(temp_response_path, "r") as temp:
        with tables.open_file(results_table_path, "a") as result_file:
            ...
            if hasattr(result_file.root, "features"):
                ...
                features = result_file.create_vlarray(
                    result_file.root, "features", tables.Float32Atom(shape=())
                )
            ...
            with tables.open_file(past_results_table_path, "r") as past_result_file:
                ...
                if index_past.any():
                    np.testing.assert_array_almost_equal(
                        feature[index], past_feature[index_past], 4
                    )
    ...
    with tables.open_file(temp_response_path, "r") as temp:
        with tables.open_file(results_table_path, "a") as result_file:
            ...
            if hasattr(result_file.root, "test_names"):
                ...
                test_names = result_file.create_table(
                    "/", "test_names", {"name": tables.StringCol(300)}
                )
                test_names.cols.name.createIndex()
...
def isEqualXMLElement(element_a, element_b):
    ...
    for name, value in list(element_a.attrib.items()):
        if is_number(element_b.attrib.get(name)) and is_number(value):
            np.testing.assert_approx_equal(
                float(element_b.attrib.get(name)), float(value), 6
            )
        else:
            assert (
                element_b.attrib.get(name) == value
            ), "Attribute of element A %s != attribute of element B %s" % (
                value,
    ...
    if element_a.tag == "value":
        ...
        assert element_a.text == element_b.text, "Text: %s != %s" % (
            element_a.text,
            element_b.text,
        )
    ...
    assert len(subelement_a) == len(subelement_b), "%s != %s" % (
        len(subelement_a),
        len(subelement_b),
    )

    for subele_a, subele_b in zip(
        sorted(subelement_a, key=lambda x: x.attrib.get("image")),
        sorted(subelement_b, key=lambda x: x.attrib.get("image")),
    ):
...
def isCSVEqual(doc_a, doc_b):
    ...
    assert len(parsed_csv_a) == len(parsed_csv_b), "%s!=%s" % (
        len(parsed_csv_a),
        len(parsed_csv_b),
    )

    # check if titles are the same
    assert parsed_csv_a[0] == parsed_csv_b[0], "%s != %s" % (
        (
            "%s..." % parsed_csv_a[0][:30]
            if len(parsed_csv_a[0]) > 30
            else parsed_csv_a[0]
        ),
        (
            "%s..." % parsed_csv_b[0][:30]
            if len(parsed_csv_b[0]) > 30
            else parsed_csv_b[0]
        ),
    )
    ...
    for row_a, row_b in zip(sorted_csv_rows_a, sorted_csv_rows_b):
        ...
        for ncol in range(1, len(row_a)):
            if parsed_csv_a[0][ncol] == "descriptor":
                ...
                np.testing.assert_array_almost_equal(
                    element_a_array, element_b_array, decimal=4
                )
            elif is_number(row_a[ncol]) and is_number(row_b[ncol]):
                np.testing.assert_approx_equal(
                    float(row_a[ncol]), float(row_b[ncol]), 6
                )
            else:
                assert row_a[ncol] == row_b[ncol], "%s != %s" % (
                    (
                        "%s..." % row_a[ncol][:30]
                        if len(row_a[ncol]) > 30
                        else row_a[ncol]
                    ),
                    (
                        "%s..." % row_b[ncol][:30]
                        if len(row_b[ncol]) > 30
                        else row_b[ncol]
                    ),
                )
...
def isHDFEqual(path_hdf_a, path_hdf_b):
    ...
    with tables.open_file(path_hdf_a, "r") as h5file_a:
        with tables.open_file(path_hdf_b, "r") as h5file_b:
            ...
            assert set(table_a.colnames) == set(table_b.colnames), "%s!=%s" % (
                table_a.colnames,
                table_b.colnames,
            )

            sorted_resource_a = sorted(
                zip(list(range(len(table_a))), table_a[:]["image"]), key=lambda x: x[1:]
            )
            sorted_resource_b = sorted(
                zip(list(range(len(table_b))), table_b[:]["image"]), key=lambda x: x[1:]
            )

            for idx_a, idx_b in zip(
                list(zip(*sorted_resource_a))[0], list(zip(*sorted_resource_b))[0]
            ):
                for colname in table_a.colnames:
                    if colname == "feature":
                        np.testing.assert_array_almost_equal(
                            table_a[idx_a][colname], table_b[idx_b][colname], 4
                        )
                    else:
                        assert (
                            table_a[idx_a][colname] == table_b[idx_b][colname]
                        ), "%s!=%s" % (table_a[idx_a][colname], table_b[idx_b][colname])
