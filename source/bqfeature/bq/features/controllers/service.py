...
from bqcore.bq.core.service import ServiceController
from bqcore.bq.util.locks import Locks
from bqcore.bq.util.mkdir import _mkdir
from bqfeature.bq.features.controllers.ID import ID
from bqfeature.bq.features.controllers.Feature import BaseFeature
from bqfeature.bq.features.controllers.utils import mex_validation
from bqfeature.bq.features.controllers.TablesInterface import (
    CachedRows,
    WorkDirRows,
    CachedTables,
    WorkDirTable,
...
from bqfeature.bq.features.controllers.var import (
    FEATURES_TABLES_FILE_DIR,
    EXTRACTOR_DIR,
    FEATURES_WORK_DIR,
)
...
class Feature_Archive(dict):
    ...
    def __init__(self):
        ...
        extractors = [
            name
            for module_loader, name, ispkg in pkgutil.iter_modules([EXTRACTOR_DIR])
            if ispkg
        ]
        for module in extractors:
            try:
                extractor = importlib.import_module(
                    "bq.features.controllers.extractors." + module + ".extractor"
                )  # the module needs to have a file named
                for n, item in inspect.getmembers(
                    extractor
                ):  # extractor.py to import correctly
                    if (
                        inspect.isclass(item)
                        and issubclass(item, BaseFeature)
                ...
                Exception
            ) as e:  # need to pick a narrower error band but not quite sure right now
                log.warning("Failed to import: %s reason %s ", module, e)
                # log.debug('Failed to import: %s\n%s'%(module, traceback.format_exc()))
                continue
...
class ResourceList(object):
    ...
    def __init__(self, feature_name, format_name, **kw):
        ...
        try:
            ...
            raise FeatureServiceError(
                404, "Feature: %s requested was not found" % feature_name
            )
    ...
    def path_in_workdir(self):
        ...
        return os.path.join(
            FEATURES_WORK_DIR, self.feature.name, filename[0], "%s.h5" % filename
        )
    ...
    def append(self, resource):
        ...
        resource_hash = self.feature.hash_resource(
            resource
        )  # the hash will change if redirects
        if resource_hash not in self.resource_list:
            self.resource_list[resource_hash] = ResourceRequest(
                feature_resource=resource, exception=exception
            )
    ...
    def add_exc(self, resource_hash, exception):
        ...
        if resource_request is None:
            ...
            self.resource_list[resource_hash] = ResourceRequest(
                feature_resource=resource_request, exception=exception
            )
            return True
    ...
    def __getitem__(self, index):
        ...
        return (
            resource_hash,
            resource_request.feature_resource,
            resource_request.exception,
        )
...
def clean_url(url):
    ...
    if isinstance(url, list):
        if len(url) == 1:
            ...
            raise FeatureServiceError(
                400, "Request Error: Only one resource per resource type"
            )
...
def parse_request(feature_name, format_name="xml", method="GET", **kw):
    ...
    if method == "POST" and 1 > request.headers["Content-type"].count("xml/text"):
        if not request.body:
            raise FeatureServiceError(
                400, "Document Error: No body attached to the POST"
            )
        try:
            ...
            raise FeatureServiceError(
                400, "Document Error: document was not formatted correctly"
            )
        ...
        def append_resource_list(feature_node):
            ...
            resource = FeatureResource(
                image=clean_url(query.get("image", "")),
        ...
        if (
            len(resource_list) < 1
        ):  # checks to see if no resourses are there or the format was not proper resulting in no elements
            raise FeatureServiceError(400, "No resources were found!")

    elif method == "GET":
        resource = FeatureResource(
            image=clean_url(kw.get("image", "")),
            mask=clean_url(kw.get("mask", "")),
            gobject=clean_url(kw.get("gobject", "")),
        )
        if resource == ("", "", ""):
            raise FeatureServiceError(
                400, "Request Error: No excepted resources were provided"
            )
        resource_list.append(resource)
...
def operations(resource_list):
    ...
    if (
        resource_list.check_response_in_workdir() is False
    ):  # skip to format reponse if true
            ...
            for results in table.find(query_plan):
                for row_exists, id in results:
                    ...
                    if row_exists is True:
                        log.debug(
                            "Returning Resource: %s from cache"
                            % str(request_resource.feature_resource)
                        )
                    else:
                        log.debug(
                            "Resource: %s was not found in the table"
                            % str(request_resource.feature_resource)
                        )
                        try:
                            ...
                            FeatureExtractionError
                        ) as feature_extractor_error:  # if error occured the exception is added to the resource
                            if not resource_list.add_exc(id, feature_extractor_error):
                                log.debug(
                                    "Exception: Resource ID: %s not found" % (str(id))
                                )
                                raise FeatureServiceError(
                                    500, "Resource hash was not found"
                                )
                            log.debug(
                                "Exception: Error Code %s : Error Message %s"
                                % (
                                    str(feature_extractor_error.code),
                                    feature_extractor_error.message,
                                )
                            )
...
def format_response(resource_list):
    ...
    if resource_list.check_response_in_workdir() is False:
        ...
        table = WorkDirTable(feature).set_path(
            resource_list.path_in_workdir()
        )  # returns unindexed table from the workdir
...
class Format(object):
    ...
    def _make_uri(feature, resource):
        ...
        query = []
        if resource.image:
            query.append("image=%s" % resource.image)
        if resource.mask:
            query.append("mask=%s" % resource.mask)
        if resource.gobject:
            query.append("gobject=%s" % resource.gobject)
        query = "&".join(query)
        return "%s://%s/features/%s/xml?%s" % (
            request.scheme,
            request.host,
            feature.name,
            query,
            ...
            for rows, hash in query:
                ...
                if error is None:
                    if rows is not None:  # a feature was found for the query
                        ...
                        yield self.construct_error_row(
                            idx,
                            self.feature,
                            FeatureExtractionError(
                                resource, 404, "The feature was not found in the table."
                            ),
                        )
                        idx += 1
                else:
    ...
    def _construct_from_workdir(self, table, resource_list, feature):
        ...
        for row, status in table.get():
            resource = FeatureResource(
                image=row["image"], mask=row["mask"], gobject=row["gobject"]
            )
            if status["status"] == 200:
                ...
                yield self.construct_error_row(
                    ...
                    FeatureExtractionError(
                        resource,
                        status["status"],
                        "An error occured during feature extraction",
                    ),
                )
                idx += 1
...
class Xml(Format):
    ...
    def construct_row(idx, feature, resource, row):
        ...
        subelement = etree.Element(
            "feature",
            uri=uri,
            type=str(feature.name),
            value=",".join("%g" % item for item in row["feature"]),
        )
    ...
    def construct_error_row(idx, feature, error):
        ...
        etree.SubElement(
            ...
            "tag",
            name="error",
            value="%s:%s" % (str(error.code), error.message),
        )
        return subelement
...
class Csv(Format):
    ...
    def construct_row(idx, feature, resource, row):
        ...
        """
        value_string = ",".join(
            "%g" % i for i in row["feature"]
        )  # parses the table output and returns a string of the vector separated by commas
        resource_uris = [
            resource.image or "",
            resource.mask or "",
            resource.gobject or "",
        ...
        line = str(
            ...
            % (
                ",".join(
                    ...
                    + [""]
                    + parameter
                    + ["%s:%s" % (str(error.code), str(error.message))]
                ),
                os.linesep,
            )
        )
...
class Hdf(Format):
    ...
    def return_from_tables(self, table, resource_list):
        ...
        for feature, status in self._construct_from_cache(
            table, resource_list, self.feature
        ):
            row_list.append(feature)
            status_list.append(status)

        # writing the output to an uncached table
        workdir_feature_table = WorkDirTable(self.feature).set_path(
            resource_list.path_in_workdir()
        )
...
class LocalPath(Format):
    ...
    def return_from_tables(self, table, element_list, **kw):
        ...
        for hash in element_list:
            localpath.append(
                (os.path.join(table.get_path(), hash[: self.feature.hash]), hash)
            )
        return localpath
...
class FeatureDoc:
    ...
    def feature_server(self):
        ...
        command = etree.SubElement(
            ...
            "command",
            name="FEATURE_NAME",
            value="Documentation of specific feature",
        )
        command = etree.SubElement(
            resource, "command", name="list", value="List of available features"
        )
        command = etree.SubElement(
            resource, "command", name="formats", value="List of output formats"
        )
        command = etree.SubElement(
            resource,
            "command",
            name="format/FORMAT_NAME",
            value="Documentation of specific format",
        )
        command = etree.SubElement(
            resource,
            "command",
            name="/FEATURE_NAME/FORMAT_NAME?image|mask|gobject=URL[&image|mask|gobject=URL]",
            value="Returns feature in format specified",
        )
        command = etree.SubElement(
            resource,
            "attribute",
            name="resource",
            value="The name of the resource depends on the requested feature",
        )
    ...
    def feature_list(self):
        ...
        for featuretype in list(FEATURE_ARCHIVE.keys()):
            ...
            if feature_module.library not in feature_library:
                feature_library[feature_module.library] = etree.SubElement(
                    resource, "library", name=feature_module.library
                )
    ...
    def feature(self, feature_name):
        ...
        if feature_module.additional_resource is not None:
            xml_attributes["additional_resource"] = ",".join(
                feature_module.additional_resource
            )
...
class featuresController(ServiceController):
    ...
    def _default(self, *args, **kw):
        ...
        if not args and request.method == "GET":
            ...
            log.info(
                "Content-Type: %s  Returning Feature List" % (header["Content-Type"])
            )
            ...
            try:
                ...
                header = {"Content-Type": "text/xml"}
                log.info(
                    "Content-Type: %s Returning Feature Info: %s"
                    % (header["Content-Type"], args[0])
                )
        ...
        # calculating features
            ...
            try:
                ...
                log.info(
                    "Content Type: %s  Returning Feature: %s"
                    % (header["Content-Type"], args[0])
                )
    ...
    def formats(self, *args):
        ...
        if request.method == "GET":
            if len(args) < 1:  # returning list of useable formats
                ...
                header = {"Content-Type": "text/xml"}
                log.info(
                    "Content Type: %s  Returning Format Type List"
                    % (header["Content-Type"])
                )
                ...
                try:
                    ...
                    header = {"Content-Type": "text/xml"}
                    log.info(
                        "Content-Type: %s  Returning Format Type: %s"
                        % (header["Content-Type"], args[0])
                    )
            ...
            log.error(
                "Malformed Request: Not a valid features request only excepts GET method"
            )
            abort(
                400,
                "Malformed Request: Not a valid features request only excepts GET method",
