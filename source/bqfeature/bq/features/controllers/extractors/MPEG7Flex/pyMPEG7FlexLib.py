...
import os
import inspect
from bqcore.bq.util.paths import run_path
...
def extractCLD(im, mask=np.array([]), numYCoef=64, numCCoef=28):
    ...
    _MPEG7FlexLib.computeCLD(
        im, numYCoef, numCCoef, int(rows), int(cols), hasmask, mask, result
    )
    return result
