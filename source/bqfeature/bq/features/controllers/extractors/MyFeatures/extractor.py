...
from bqapi.bqapi import BQServer
import random
from bqfeature.bq.features.controllers.utils import (
    image2numpy,
    except_image_only,
    fetch_resource,
)
from bqfeature.bq.features.controllers import Feature
from bqfeature.bq.features.controllers.exceptions import FeatureExtractionError
    ...
    def calculate(self, resource):
        ...
        if resource.mask:
            raise FeatureExtractionError(
                resource, 400, "Mask resource is not accepted."
            )
        if resource.image:
            raise FeatureExtractionError(
                resource, 400, "Image resource is not accepted."
            )
        ...
        if poly_xml.tag == "polygon":
            ...
            raise ValueError(
                "Polygon not found: Must be a polygon gobject"
            )  # an excpetion instead of abort so work flow is not interupted
