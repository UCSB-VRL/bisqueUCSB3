...
def gabor_gen(height, width, s, n, scales=4, orientations=6, Ul=0.05, Uh=0.5):
    ...
    sigmaV = (
        np.tan(np.pi / (2 * orientations))
        * (Uh + z)
        / np.sqrt(2 * np.log(2) - z * z / (sigmaU * sigmaU))
    )
    ...
    G = (
        coef
        * (a ** (-s))
        * np.exp(-0.5 * ((X * X) / (sigmaX * sigmaX) + (Y * Y) / (sigmaY * sigmaY)))
    )
    ...
    for s in np.arange(0, scales):
        for o in np.arange(0, orientations):
            ...
            result = np.fft.irfft2(
                np.fft.rfft2(gabor) * np.fft.rfft2(im, gabor.shape)
            )  # faster
