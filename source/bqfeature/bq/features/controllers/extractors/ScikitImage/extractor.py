...
class HOG2(Feature.BaseFeature):
    ...
    def calculate(self, resource):
        ...
        im = image2numpy(resource.image, remap="gray")
        feature = hog(
            im, orientations=4, pixels_per_cell=(8, 8), cells_per_block=(2, 2)
        )
        feature = np.reshape(
            feature, (feature.size / 16, 16)
...
def rotation_invariant_moments(im):
    ...
    I5 = (
        ...
        * (n30 + n12)
        * (
            ((n30 + n12) ** 2 - 3 * (n21 + n30) ** 2)
            + (3 * n21 - n03) * (n21 - n03) * (3 * (n30 + n12) ** 2 - (n21 + n30) ** 2)
        )
    )
    I6 = (n20 - n02) * ((n30 + n12) ** 2 - (n21 + n03) ** 2) + 4 * n11 * (n30 + n12) * (
        n21 + n03
    )
    I7 = (
        (3 * n21 - n03)
    ...
    I8 = n11 * ((n30 + n21) ** 2 - (n03 + n21) ** 2) - (n20 - n02) * (n30 + n21) * (
        n03 + n21
    )
