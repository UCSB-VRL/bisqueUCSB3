...
class BaseFeature(object):
    ...
    def __init__(self):
        ...
        self.cache = str2bool(
            config.get(
                "bisque.feature.%s.cache" % self.name, None
            )  # checks for specific feature
            or config.get("bisque.feature.default.cache", None)  # checks the default
            or "False"
        )  # sets a default if nothing is specified
    ...
    def hash_resource(feature_resource):
        ...
        """
        query = []
        if feature_resource.image:
            query.append("image=%s" % feature_resource.image)
        if feature_resource.mask:
            query.append("mask=%s" % feature_resource.mask)
        if feature_resource.gobject:
            query.append("gobject=%s" % feature_resource.gobject)
        query = "&".join(query)
