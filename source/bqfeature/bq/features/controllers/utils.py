...
from bqserver.bq import image_service
from bqserver.bq import data_service
from bqcore.bq.core import identity
from bqapi.bqapi import BQServer, BQCommError
from bqcore.bq.util.mkdir import _mkdir
from bqfeature.bq.features.controllers.exceptions import (
...
def mex_validation(resource):
    ...
    for name in list(resource_name):
        ...
        try:
            ...
            m = re.match(
                "\/(?P<service>[\w-]+)\/(image[s]?\/|)(?P<id>[\w-]+)", url_path
            )
            if m is not None:
                if (
                    m.group("service") == "image_service"
                    or m.group("service") == "data_service"
                ):  # check for data_service
                    if "pixels" not in url_path:  # if false requires a redirect
                        ident = m.group(
                            "id"
                        )  # seaching a plan image_service or data_service url
            ...
            raise InvalidResourceError(
                resource_url=url,
                error_code=403,
                error_message="Resource: %s Not Found" % url,
            )
...
def fetch_resource(uri):
    ...
    try:
        ...
        raise InvalidResourceError(
            resource_url=uri,
            error_code=403,
            error_message="Resource: %s Not Found" % uri,
        )
...
def image2numpy(uri, **kw):
    ...
    with tempfile.NamedTemporaryFile(
        dir=FEATURES_TEMP_DIR, prefix="image", delete=False
    ) as f:
...
def convert_image2numpy(image_path):
    ...
    try:
        ...
        raise InvalidResourceError(
            error_code=415, error_message="Not a grayscale or RGB image"
        )
        ...
        try:
            ...
            )  # try to return something, pil doesnt support bigtiff
        except IOError:
            log.debug("File type not supported!")
            raise InvalidResourceError(
                error_code=415, error_message="Unsupported media type"
            )
...
def gobject2mask(uri, im):
    ...
    """
    valid_gobject = set(
        ["polygon", "circle", "square", "ellipse", "rectangle", "gobject"]
    )
    ...
    try:
        ...
        raise FeatureExtractionError(
            None, 415, "Url: %s, was not xml for gobject" % uri
        )
    ...
    if xml.tag not in valid_gobject:
        raise FeatureExtractionError(
            None,
            415,
            "Url: %s, Gobject tag: %s is not a valid gobject to make a mask"
            % (uri, xml.tag),
        )
    ...
    if xml.tag in set(["gobject"]):
        ...
        if tag is None:
            raise FeatureExtractionError(
                None, 415, "Url: %s, Not an expected gobject" % uri
            )
    else:
        tag = xml.tag
    ...
    if tag in set(["polygon"]):
        ...
        for vertex in xml.xpath("vertex"):
            ...
            if x is None or y is None:
                raise FeatureExtractionError(
                    None, 415, "Url: %s, gobject does not have x or y coordinate" % uri
                )
            contour.append((int(float(x)), int(float(y))))
        if len(contour) < 2:
            raise FeatureExtractionError(
                None, 415, "Url: %s, gobject does not have enough vertices" % uri
            )
        #        import pdb
        #        pdb.set_trace()
    ...
    if tag in set(
        ["circle", "ellipse"]
    ):  # ellipse isnt supported really, its just a circle also
        # takes only the first 2 points
        contour = []
        for vertex in xml.xpath("vertex"):
            x = vertex.attrib.get("x")
        ...
        r = np.sqrt(
            np.square(int(float(x2)) - int(float(x1)))
...
def gobject2keypoint(uri):
    ...
    if xml.tag == "circle":
        ...
        if (
            center_x is None
            or center_y is None
            or outer_vertex_x is None
            or outer_vertex_y is None
        ):
            raise FeatureExtractionError(
                None, 415, "Url: %s, gobject does not have x or y coordinate" % uri
            )
        ...
        r = np.sqrt(
            np.square(int(float(outer_vertex_x)) - int(float(center_x)))
