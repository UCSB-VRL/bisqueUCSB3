...
class TablesLock(object):
    ...
    def __init__(self, filename, mode="w", failonexist=False, *args, **kwargs):
        ...
        if mode in set(["w", "a"]):  # write lock
            self.bq_lock = Locks(
                None, self.filename, failonexist=failonexist, mode=mode + "b"
            )
        else:  # read lock
            self.bq_lock = Locks(
                self.filename, None, failonexist=failonexist, mode=mode + "b"
            )
    ...
    def _open_table(self):
        ...
        try:
            if os.name == "nt" and self.mode == "w":
                ...
                self.h5file = tables.open_file(
                    self.filename, "r+", *self.args, **self.kwargs
                )
...
class Rows(object):
    ...
    def _calculate_row(self, feature_resource):
        try:
            ...
            log.exception(
                "Calculation Error: URI:%s  %s Feature failed to be calculated"
                % (str(feature_resource), self.feature.name)
            )
            raise FeatureExtractionError(
                feature_resource,
                500,
                "Internal Server Error: Feature failed to be calculated",
            )
...
class WorkDirRows(Rows):
    ...
    def push(self, request_resource):
        ...
        if request_resource.exception:
            rows, status = self.construct_error_row(
                self.feature, request_resource.exception
            )
            log.debug("rows %s", rows)
        else:
            try:
                rows, status = self.construct_row(
                    self.feature, request_resource.feature_resource
                )
                log.debug("rows %s", rows)
        ...
        if (
            "feature" not in self.row_queue
        ):  # checking the first few element on the hash
            self.row_queue["feature"] = (
                queue.Queue()
            )  # build queue since none were found
        self.row_queue["feature"].put(
            (rows, status)
        )  # the row is pushed into the queue
    ...
    def write_to_table(self, filename, func=None):
        ...
        with TablesLock(filename, "w", failonexist=True) as h5file:
            if h5file:
                try:
                    ...
                    for table_name, columns, indexed in self.table_plan:
                        table = h5file.create_table(
                            "/", table_name, columns, expectedrows=1000000000
                        )
                ...
                ):  # race condtion may occur, stop writing if table already contains table
    ...
    def read_from_table(self, filename, func):
        ...
        while 1:
            ...
            if attempts > FAILED_LOCK_ATTEMPTS:
                raise FeatureServiceError(
                    error_message="Failed to lock the table for the %s time"
                    % FAILED_LOCK_ATTEMPTS
                )
            else:
                log.debug(
                    "Table was not locked: %s attempting to relock the table after %s sec."
                    % (filename, LOCKING_DELAY)
                )
...
class CachedTables(Tables):
    ...
    def __init__(self, feature):
        ...
        self.table_plan.append(
            self.TablePlan("values", self.feature.cached_columns(), ["idnumber"])
        )
        ...
        for filename in list(rows.keys()):
            ...
            def func(h5file):
                ...
                while not queue.empty():
                    row = queue.get()
                    query = 'idnumber=="%s"' % str(
                        row[0][0]
                    )  # queries the hash to see if a feature has been already added
                    try:
                        next(table.where(query))  # if the list contains one element
                        log.debug(
                            "Skipping %s - already found in table" % str(row[0][0])
                        )
    ...
    def delete(self):
        ...
        for t in table_list:
            try:
                ...
                log.debug(
                    "Workdir hdf5 failed to delete @ %s"
                    % os.path.join(self.feature.path, self.feature.filename)
                )
    ...
    def __len__(self):
        ...
        log.debug(
            "feature table list %s" % str(os.path.join(self.feature.path, "*.h5"))
        )
...
class WorkDirTable(Tables):
    ...
    def __init__(self, feature):
        ...
        self.table_plan = []
        self.table_plan.append(
            self.TablePlan("values", self.feature.workdir_columns(), [])
        )
        self.table_plan.append(
            self.TablePlan("status", {"status": tables.Int16Col()}, ["status"])
        )
