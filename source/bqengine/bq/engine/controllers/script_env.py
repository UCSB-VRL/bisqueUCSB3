...
class ScriptEnvironment(BaseEnvironment):
    ...
    def create_script(self, mex):
        ...
        quoted.update(
            dict((k, quote(v)) for (k, v) in list(mex.items()) if isinstance(v, str))
        )
    ...
    def setup_environment(self, runner):
        for mex in runner.mexes:
            if mex.executable:
                ...
                runner.debug(
                    "Execute setup '%s' in %s" % (" ".join(script + ["setup"]), rundir)
                )
    ...
    def teardown_environment(self, runner):
        for mex in runner.mexes:
            if mex.executable:
                ...
                rundir = mex.get("rundir")
                runner.debug(
                    "Execute teardown '%s' in %s"
                    % (" ".join(script + ["teardown"]), rundir)
                )
                r = subprocess.call(
                    script + ["teardown"],
                    stdout=open(mex.log_name, "a"),
