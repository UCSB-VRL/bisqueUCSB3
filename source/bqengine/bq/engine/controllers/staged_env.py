...
class StagedEnvironment(BaseEnvironment):
    ...
    def _set_staging_path(self, runner, mex):
        ...
        if not mex.staging_path:
            ...
            mex.staging_path = os.path.abspath(
                os.path.expanduser(os.path.join(staging_base, mex.mex_id))
            )
    ...
    def setup_environment(self, runner, **kw):
        ...
        runner.info(
            "staged environment setup cwd=%s rundir=%s", os.getcwd(), runner.rundir
        )
        for mex in runner.mexes:
            ...
            if mex.get("files"):
                ...
                runner.debug(
                    "copying %s: %s to %s" % (mex.initial_dir, files, mex.staging_path)
                )
    ...
    def teardown_environment(self, runner, **lw):
        ...
        for mex in runner.mexes:
            ...
            runner.debug(
                "Cleaning %s with status %s", getattr(mex, "staging_path", None), status
            )
            if status == "finished":
